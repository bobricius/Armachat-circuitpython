...
while True:
    ...
    text = editor(text=message, helpText="")
    setStatusBar()
    if text=="~bsp":
    ...
    if len(text) == 1:
        if text == "h":
            ...
            cmdInfo = "[E]edit [M]Memory [L]List [R]Radio\n[C]Crypto [D]Debug [G]Ping [V]Receive all [T]SF [P]Power [B] Battery [I]Storage [A]Address to"
            ...
            ring()
            text = editor(text=str(spreading_factor),helpText="Spread factor:")
            ...
            ring()
            text = editor(text=str(tx_power),helpText="TX power:")
            ...
            cmdInfo = "Changed font"
            
        elif text == "a": #addresbook experiment         
            text = editor(text=destination,helpText="Enter address:")
